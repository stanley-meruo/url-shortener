...
export default function NavBar() {
  ...
  return (
    <nav className="fixed w-full z-50">
      <div
        className={`text-gray-500 font-semibold transition-colors duration-300 px-5 xs:px-6 sm:px-8 lg:px-14 xl:px-20 xxl:px-36 ${
          scrolled ? "bg-cyan shadow-md text-white" : "bg-transparent"
        }`}
      >
        <div className="flex items-center justify-between py-5">
          ...
          <div className="hidden md:flex gap-6">
            <Link href="/" className="block p-2 hover:text-black">
              Login
            </Link>
            <Link
              href="/"
              className={`block bg-cyan rounded-full py-2 px-4 transition-colors duration-300 hover:opacity-50 lg:px-6 ${
                scrolled ? "bg-white text-gray-500" : "text-white"
              }`}
            >
              Sign Up
            </Link>
          ...
          <div className="md:hidden">
            <button onClick={toggleMenu} className="text-gray-600">
              {isOpen ? <RiCloseFill size={32} /> : <RiMenuFill size={32} />}
            </button>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="md:hidden bg-darkviolet rounded-lg mx-4 xs:mx-6 sm:mx-8"
            initial={{ height: 0, opacity: 0 }}
            animate={{ height: "auto", opacity: 1 }}
            exit={{ height: 0, opacity: 0 }}
            transition={{ duration: 0.25 }}
          >
            <div className="text-white font-semibold p-6 xs:p-8 sm:p-10">
              ...
              <div className="h-[0.5px] bg-gray-500 w-full my-2"></div>
              <div className="grid gap-2 text-center">
                <Link href="/" className="block p-2">
                  Login
                </Link>
                <Link href="/" className="block bg-cyan rounded-full p-2">
                  Sign Up
                </Link>
              </div>
            </div>
